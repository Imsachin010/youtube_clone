...
const parseData = async(item) => {
    const channelResponse = await axios.get(`https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&id=${item.snippet.channelId}&key=${API_KEY}`)
    ...
    return {
        ...
        videoAge:timeSince(
            new DataTransfer(snippet.publishedAt)
        ),
        channelInfo: {
